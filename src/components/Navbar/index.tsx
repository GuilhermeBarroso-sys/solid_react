...
export function Navbar() {
	const [mobileDataCollapse, setMobileDataCollapse] = useState(false);
	const mobileDataCollapseUseRef = useRef<HTMLHeadingElement>(null);
	function toggleMobileDataCollapse() {
		setMobileDataCollapse(!mobileDataCollapse);
	}
	useEffect(() => {
		if(mobileDataCollapse) {

			mobileDataCollapseUseRef.current?.classList.remove("hidden");
		} else {
			mobileDataCollapseUseRef.current?.classList.add("hidden");
			...
			<nav className="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-[#222831]">
				...
				<div className="container flex flex-wrap justify-between items-center mx-auto">
					...
					<div className="flex md:order-2">
						...
						<button data-collapse-toggle="mobile-menu-3" onClick={toggleMobileDataCollapse} type="button" className="inline-flex items-center p-2 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="mobile-menu-3" aria-expanded="false">
					...
					<div ref={mobileDataCollapseUseRef} className="hidden justify-between items-center w-full md:flex md:w-auto md:order-1" id="mobile-menu-3">
